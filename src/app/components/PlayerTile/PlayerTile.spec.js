...
test('<PlayerTile />', t => {
  ...
  t.notOk(
    shallow(<Tile />).hasClass(styles.active),
    "Applies normal style"
  )

  t.ok(
    shallow(<Tile active />).hasClass(styles.active),
    "Applies active style"
  )

  const dad = shallow(<Tile name="Dad" />);
  dad.map( (i) => { 
